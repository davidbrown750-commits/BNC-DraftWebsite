...
(function () {
  "use strict";
  var FORMSPREE = "/api/form?form=pdf-config";
...
/* ---- Legacy PDF download line (2026-07-12, moved to hero 2026-08-11) ------
 * Renders the print-code chip on datasheet/manual pages: document date code
 * (Dc-...) plus a "PDF download (size)" link, styled to sit alongside the
 * Bioz citation badge in the hero rather than at the bottom of the page. The
 * file map is generated by the Legacy PDF Downloads rollout
 * (docs/pdfs/pdf-map.json). The link is gated to signed-in visitors by
 * bnc-auth.js's download gate (a.pdf-dl).
 */
(function () {
  "use strict";
  function ready(fn){ if (document.readyState !== "loading") fn(); else document.addEventListener("DOMContentLoaded", fn); }
  ready(function () {
    ...
    xhr.onload = function () {
      ...
      a.target = "_blank";
      a.rel = "noopener";
      a.setAttribute("style", "font:700 12.5px Arial,Helvetica,sans-serif;color:#0655a3;text-decoration:none;");
      ...
      // Anchor point: sit beside the Bioz badge in the hero when one exists,
      // otherwise beside the version/doc-code line, otherwise at the end of
      // the hero text column. Falls back to the old bottom-of-page placement
      // only if a page has no .doc-hero .ht at all.
      var heroCol = document.querySelector(".doc-hero .ht");
      var biozBadge = document.querySelector(".doc-hero .bioz-badge");
      var verLine = document.querySelector(".doc-hero .ver");

      if (heroCol) {
        line.setAttribute("style",
          "display:inline-flex;align-items:center;gap:10px;margin-top:14px;" +
          "background:#fff;border:1px solid #d7e0ea;border-left:3px solid #0655a3;" +
          "border-radius:6px;padding:8px 14px;");
        var code = document.createElement("span");
        code.setAttribute("style", "font:400 11px Arial,Helvetica,sans-serif;color:#8b97a8;");
        code.textContent = m.c || "";
        if (code.textContent) line.appendChild(code);
        line.appendChild(a);
        if (biozBadge && biozBadge.parentNode) {
          biozBadge.insertAdjacentElement("afterend", line);
        } else if (verLine && verLine.parentNode) {
          verLine.insertAdjacentElement("afterend", line);
        } else {
          heroCol.appendChild(line);
        }
      } else {
        ...
        var code2 = document.createElement("span");
        code2.textContent = m.c || "";
        a.style.marginLeft = "auto";
        line.appendChild(code2);
        line.appendChild(a);
...
(function(){
  ...
  ready(function(){
    ...
    function mk(tag, attrs, html){ var e=document.createElement(tag); if(attrs) for(var k in attrs) e.setAttribute(k, attrs[k]); if(html!=null) e.innerHTML=html; return e; }
    var NAVY = "#113163";
    function biozModal(href){
      var ov = document.getElementById("bioz-modal");
      if (!ov) {
        ov = mk("div", {id:"bioz-modal", style:
        ...
        var head = mk("div", {style:
          "display:flex;align-items:center;gap:10px;background:"+NAVY+";color:#fff;padding:12px 18px"},
        ...
        var body = mk("div", {id:"bioz-modal-body", style:"flex:1 1 auto;overflow:auto;background:#f6f8fb"});
