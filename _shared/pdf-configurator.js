 ...
 *
 * 2026-08-12: also renders a second, button-styled entry point in the hero
 * (see "Hero PDF download button" below) - this one is additive, the bottom
...
(function () {
  ...
  ready(function () {
    ...
    xhr.onload = function () {
      ...
      /* ---- Hero PDF download button (2026-08-12) -----------------------
       * Additional entry point styled to match .hero-cfg-btn (the blue
       * "Model Configurator" button) and placed right beside it when a
       * page has one, so the download is visible above the fold too.
       */
      var BTN_BG = "#0655a3", BTN_BG_HOVER = "#113163";
      var heroBtn = document.createElement("a");
      heroBtn.className = "pdf-dl";
      heroBtn.href = m.f;
      heroBtn.target = "_blank";
      heroBtn.rel = "noopener";
      if (m.c) heroBtn.title = m.c + (m.s ? " · " + m.s : "");
      heroBtn.setAttribute("style",
      ...
      var cfgBtn = document.querySelector(".doc-hero .hero-cfg-btn");
      var biozBadge = document.querySelector(".doc-hero .bioz-badge");
      var verLine = document.querySelector(".doc-hero .ver");

      if (heroCol) {
        if (cfgBtn && cfgBtn.parentNode) {
          heroBtn.style.marginLeft = "12px";
          cfgBtn.insertAdjacentElement("afterend", heroBtn);
        } else if (biozBadge && biozBadge.parentNode) {
          biozBadge.insertAdjacentElement("afterend", heroBtn);
        } else if (verLine && verLine.parentNode) {
          verLine.insertAdjacentElement("afterend", heroBtn);
        } else {
          heroCol.appendChild(heroBtn);
        }
      }
