...
// `cid` (optional) is the contact id from the form-notification email; if searchByEmail
// hasn't indexed a freshly-created contact yet, we fall back to that id, but ONLY after
// confirming the contact at that id actually carries this email (so a tampered link can't
// delete an arbitrary record). Returns {found, id, action:"deleted"|"flagged"|"error", via}.
async function removeSpamContactByEmail(email, cid) {
  let contact = await findContactByEmail(email);
  let via = "search";
  if (!contact && cid) {
    try {
      const numeric = Number(String(cid).split("-")[0]);
      if (numeric > 0) {
        const c = await rpc("getContact", { contactId: numeric });
        const emails = (c && c.emails) || [];
        const match = emails.some((e) => String((e && e.value != null) ? e.value : e).toLowerCase() === String(email).toLowerCase());
        if (c && !c.deletedTime && match) { contact = { id: c.id || (numeric + "-contacts") }; via = "id"; }
      }
    } catch (_) {}
  }
  ...
  try {
    ...
    return { found: true, id, action: "deleted", via };
  } catch (e) {
    try {
      await flagContactSpam(id);
      return { found: true, id, action: "flagged", via, deleteError: e.rpc || e.message };
    } catch (e2) {
      return { found: true, id, action: "error", via, error: e2.rpc || e2.message };
    }
  }
}
